...
CREATE TABLE IF NOT EXISTS emergencia (id serial primary key, id_insitucion serial, nombre text, descripcion text unique
                                    , fecha date, reqs_grupales text, reqs_individuales text,longitude NUMERIC(5,3), latitude NUMERIC(5,3), geom geometry(Point,4326));
...
CREATE TABLE IF NOT EXISTS tarea (id serial primary key, id_emergencia serial, nombre text, descripcion text, fecha date, 
                                requerimientos text, longitude NUMERIC(5,3), latitude numeric(5,3), geom geometry(Point,4326));
